...
export class SsoApi {
    ...
    async login(url:IUrlBase, username: string, password: string, loginData: SsoLoginData): Promise<any> {
        let headers = {};
        ...
        const encryptedPassword = new Croypto().desEncrypt(loginData.crypto || '', password);
        const data = {
            username: username,
